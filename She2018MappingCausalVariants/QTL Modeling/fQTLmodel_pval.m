...
function [sensitivity,specificity,truePosMag,falseNegMag] = fQTLmodel(Trials,nSites,cycles,nSegregants,genomeSize,RR,pSites,noise,pval)
...
%         LOD{3}(i) = -(nSegregants)'.*log(1-r{3}^2)/(2*log(10)); % perfect genotyping + phenotyping
    ...
    % Forward selection
    [b_fwselection,se,pval,inmodel,stats,nextstep,history] = stepwisefit(genotypes,phenotypes,'penter',pval,'display','off');
