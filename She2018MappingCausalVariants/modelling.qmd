...
def simulate_recombination_once(parents, phases, recombination_rate):
    ...
    for i in range(0, len(indices), 2):
        ...
        recombination_sites = np.sort(np.random.choice(num_variants, size=num_swaps, replace=False))
        start = recombination_sites[0]
        # swap a segment then jump two recombination sites forward
        for isite in range(0,num_swaps,2):
            if isite == num_swaps-1:
                end = num_variants
            else:
                end = recombination_sites[isite+1]
            # print("isite = ",isite,"; num_swaps = ",num_swaps)
            # print("recombination sites = ",recombination_sites,"; start = ",start,"; end = ",end)
            ...
            if isite < num_swaps-2:
                start = recombination_sites[isite+2]
            # print("child1_phase: ",child1_phase)
            # print("child2_phase: ",child2_phase)
