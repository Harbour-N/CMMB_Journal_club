...
def gillespie(t_start,t_stop,omega, lam, population, state_changes,l,logl,time_points):
    ...
    while t<t_stop:
        ...
        state=stats.rv_discrete(values=([0,1,2],[prob[0]/np.sum(prob), prob[1]/np.sum(prob), prob[2]/np.sum(prob)])).rvs() #choosing a state change from a discrete distribution 
        t=t+del_t #updating the time
        population=population+state_changes[state] #updating the population
        l.append(p+1)
        logl.append(np.log(p+1))
...
for i in range(num_sims):
    population,l,logl,time_points=gillespie(t_start=start_time,t_stop=final_time,omega=0.07,lam=0.7,population=init_population, state_changes=state_changes, l=l,logl=logl, time_points=time_points)
    plt.plot(time_points,l)
    l = []
    time_points = []
    logl = []
