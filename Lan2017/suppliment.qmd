...
plt.title(f"Disitribution of colony sizes at t = {final_time}, number of samples = {num_sims}")
...
plt.semilogy(n,neg_binomial(n,final_time,omega,lam), 'r--')
plt.semilogy(n,analytical_sol(n,final_time,omega,lam), 'g--')
...
plt.semilogy(n,neg_binomial(n,final_time,omega,lam))
...
all_time_points = np.concatenate(all_time_points)
all_colony_sizes = np.concatenate(all_colony_sizes)

# Interpolate the colony sizes
interp_func = interp1d(all_time_points, all_colony_sizes, kind='linear', fill_value='extrapolate')

# Create a new time array for interpolation
t_interp = np.arange(0, final_time, 1)

# Calculate the interpolated colony sizes
interpolated_colony_sizes = interp_func(t_interp)
plt.plot(t_interp, interpolated_colony_sizes, label='Interpolated Mean', color='blue')
